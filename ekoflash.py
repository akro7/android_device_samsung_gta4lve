...
# --- AKRO-X ULTRA CYBER THEME (CLASSIC EDITION) ---
...
class EkoFlashGUI:
    def __init__(self, root):
        ...
        self.root.geometry("950x850") # تم زيادة الطول لاستيعاب الأزرار الجديدة
    ...
    def setup_layout(self):
        ...
        tk.Label(header, text="☐ EKO FLASH PRO v2.0 ☐", bg=BG_MAIN, fg=TEXT_BLUE, 
                 font=("Consolas", 18, "bold")).pack()
        tk.Label(header, text="AKRO-X ULTRA-CORE", bg=BG_MAIN, fg=FG_WHITE, 
        ...
        self.btn_fastboot = tk.Button(mode_frame, text="FASTBOOT MODE", bg=BTN_BLUE, fg=FG_WHITE,
                                      font=("Consolas", 10, "bold"), width=20, relief="flat", command=lambda: self.switch_mode("FASTBOOT"))
        ...
        self.btn_odin = tk.Button(mode_frame, text="ODIN / HEIMDALL MODE", bg=BTN_DARK, fg=FG_WHITE,
                                  font=("Consolas", 10, "bold"), width=20, relief="flat", command=lambda: self.switch_mode("ODIN"))
        ...
        for i, p in enumerate(parts):
            ...
            tk.Label(row, text=f"[{i+1}] {p.upper()}", bg=BG_MAIN, fg=TEXT_BLUE, 
                     width=16, anchor="w", font=("Consolas", 10, "bold")).pack(side="left")
            ...
            ent.pack(side="left", width=45, ipady=5, padx=15)
            
            # Buttons
            self.create_btn(row, "Browse", lambda x=p: self.browse(x), 10, BTN_ORANGE, "#000").pack(side="left", padx=5)
            self.create_btn(row, "Flash", lambda x=p: self.flash(x), 10, BTN_BLUE, FG_WHITE).pack(side="left", padx=5)

        # --- EXTRA DEVELOPER OPERATIONS ---
        extra_frame = tk.Frame(self.root, bg=BG_MAIN)
        extra_frame.pack(pady=10)
        
        self.create_btn(extra_frame, "Flash via Sideload", self.adb_sideload, 20, "#555", FG_WHITE).pack(side="left", padx=5)
        ...
        chk_style = {"bg": BG_MAIN, "fg": TEXT_BLUE, "selectcolor": "#111", "activebackground": BG_MAIN, "activeforeground": TEXT_BLUE}
        ...
        log_label.pack(fill="x", padx=30, anchor="w")
        
        self.log_widget = tk.Text(self.root, height=8, bg="#080808", fg=FG_WHITE, font=("Consolas", 9),
    ...
    def flash(self, p):
        ...
        def task():
            ...
            # دعم وضع التفليش بناءً على التبويب
            if self.current_mode == "FASTBOOT":
                cmd = ["fastboot", "flash", p, path]
            else:
                # استخدام صيغة Heimdall لأنها المقابل لـ Odin عبر أوامر الـ CMD
    ...
    def flash_all(self):
        ...
        # هنا يمكنك وضع مسار السكربت لتفليش الجميع برمجياً أو حلقة تكرار للمسارات
